	...
	public class FastMath
	{
		...
		static public Vector4 Interpolate(ref Vector4 a0, ref Vector4 a1, ref Vector4 a2, ref Vector4 a3, ref Vector4 b0, ref Vector4 b1, ref Vector4 b2, ref Vector4 b3, float fx, float invFx, float fy, float invFy, float t)
		{
			if(fx != 0.0f)
			{
				...
				float Bx = B0x * fy + A1x * invFy;
				float By = B0y * fy + A1y * invFy;
				float Bz = B0z * fy + A1z * invFy;
				float Bw = B0w * fy + A1w * invFy;
...
#if WATER_SIMD
		static public Vector4f Interpolate(Vector4f a0, Vector4f a1, Vector4f a2, Vector4f a3, Vector4f b0, Vector4f b1, Vector4f b2, Vector4f b3, float invFxf, float fxf, float invFyf, float fyf, float tf)
		{
			if(fxf != 0.0f)
			{
